...
const openNewModal = () => {
  document.getElementById("create").classList.toggle('is-active');
}
...
const createApt = (event, tenant) => {
  ...
  //console.log(customer_name, email, start_date, selectedService, selectedTime)
  sendAptData({
    first_name,
    last_name,
    email,
...
const sendAptData = (appointment) => {
  ...
  .then(data => {
    ...
    document.getElementById("message-text").innerHTML = "<strong>Appointment successfully created. An email was sent to the email address provided.</strong>"
  })
}
...
const closeModal = (id, event) => {
  ...
  document.getElementById(id).classList.remove('is-active')
    ///ele.parentElement.classList.toggle('is-active');
}
...
const openModal = (modalId, appointmentId) => {
    ...
    //     buildModal({name, start_date: data.start_date, start_time: data.start_time });
    // });
...
const addCancelMessage = (id, name, date, time) => {
  ...
  message.innerHTML = `Are you sure want to <strong>cancel</strong> appointment for <strong>${name}</strong> on <strong>${moment(date).format("dddd, MMMM D")}</strong> at <strong>${time}</strong>?`;
...
const sendEdit = (appointment, id) => {
  ...
  .then(data => {
    console.log(data)
    document.getElementById("notification").style.display = "block";
    document.getElementById("message-text").innerHTML = "<strong>Appointment successfully updated.</strong>"
  })
}